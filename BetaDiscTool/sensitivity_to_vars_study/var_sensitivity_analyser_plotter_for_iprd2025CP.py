...
cfd_indices = list(range(1))
width_indices = list(range(1))

vars_to_analyze = [
  'pmax', 'tmax', 'area_new',
  'uarea_new', 'risetime', 'tot',
  'rms'
]

for i in width_indices:
  vars_to_analyze.append(f'width_{(1)*50}pc')
...
for entry in range(n_entries):
  ...
  for ch in [1, 2]:
    ...
    for var in [
      'pmax', 'tmax',
      'area_new', 'uarea_new', 'risetime', 'tot',
      'rms'
    ]:
    ...
    for i in width_indices:
      varname = f'width_{(i+1)*50}pc'
      val = width_vec[4] if i < len(width_vec) else np.nan
...
plt.barh(y - bar_width/2, auc1, height=bar_width, color='dodgerblue', label='20 μm (Ch 1) 200 V')
plt.barh(y + bar_width/2, auc2, height=bar_width, color='red', label='20 μm (Ch 2) 150 V')

plt.yticks(y, latex_labels,fontsize=24)
plt.xticks(fontsize=24)
plt.xlabel("ROC AUC",fontsize=24)
plt.xlim(0,1)
